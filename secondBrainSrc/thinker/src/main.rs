use activity_tracker_common::{
    ActivitySummary, EventStore, GeneralDbClient, SummaryStore, TimescaleClient,
};
...
async fn main() -> Result<(), Box<dyn Error>> {
    let events_db =
        TimescaleClient::new("postgres://postgres:postgres@localhost:5435/second_brain").await?;
